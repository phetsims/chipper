 ...
 */
function createMipmap( source, maxLevel, quality ) {
  return new Promise( ( resolve, reject ) => {
    ...
    if ( source.startsWith( jpgHeader ) ) {
      loadJPEG();
    }
    else if ( source.startsWith( pngHeader ) ) {
      loadPNG();
    }
    else {
      throw new Error( 'unknown image type: ' + source.slice( 50 ) );
    }

    // Loads / decodes the initial JPEG image, and when done proceeds to the mipmapping
    function loadJPEG() {
      const imageData = jpeg.decode( new Buffer( source.slice( jpgHeader.length ), 'base64' ) );
    ...
    function loadPNG() {
      ...
      const src = bufferToStream( new Buffer( source.slice( pngHeader.length ), 'base64' ) );
    ...
    function startMipmapCreation() {
      ...
      function encodingComplete() {
        grunt.log.debug( 'mipmapped ' + ( maxLevel >= 0 ? ' to level ' + maxLevel : '' ) + ' with quality: ' + quality );
      ...
      function encodeLevel( level ) {
        ...
        outputPNG( mipmaps[ level ].data, mipmaps[ level ].width, mipmaps[ level ].height, function( buffer ) {
          ...
          mipmaps[ level ].pngURL = pngHeader + buffer.toString( 'base64' );
          if ( --encodeCounter === 0 ) {
            encodingComplete();
          }
        } );
        ...
        if ( !hasAlpha ) {
          ...
          outputJPEG( mipmaps[ level ].data, mipmaps[ level ].width, mipmaps[ level ].height, quality, function( buffer ) {
            ...
            mipmaps[ level ].jpgURL = jpgHeader + buffer.toString( 'base64' );
            if ( --encodeCounter === 0 ) {
              encodingComplete();
            }
          } );
...
module.exports = function( source, map, meta ) {
  ...
  // NOTE: We're grabbing the result of the image loader. yuk
  let base64 = source.slice( source.indexOf( 'data:image' ) );
  base64 = base64.slice( 0, base64.indexOf( '"' ) );

  createMipmap( base64, 4, 98 ).then( result => {
    const js = `
var mipmaps = ${JSON.stringify( result.map( ( { width, height, url } ) => ( { width: width, height: height, url: url } ) ) )};
